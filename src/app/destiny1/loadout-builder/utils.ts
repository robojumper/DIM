...
function getBonusType(armorpiece: D1ItemWithNormalStats): string {
  ...
  return (
    (armorpiece.normalStats[D1_StatHashes.Intellect].bonus > 0 ? 'int ' : '') +
    (armorpiece.normalStats[D1_StatHashes.Discipline].bonus > 0 ? 'dis ' : '') +
    (armorpiece.normalStats[D1_StatHashes.Strength].bonus > 0 ? 'str' : '')
  );
}
...
export function calcArmorStats(
  ...
  for (const armor of pieces) {
    const int = armor.item.normalStats![D1_StatHashes.Intellect];
    const dis = armor.item.normalStats![D1_StatHashes.Discipline];
    const str = armor.item.normalStats![D1_StatHashes.Strength];
    ...
    if ([2820418554, 2122538507, 2300914892].includes(armor.item.hash)) {
      stats[D1_StatHashes.Intellect].value += int.base;
    } else {
      stats[D1_StatHashes.Intellect].value += int[scaleType];
      stats[D1_StatHashes.Discipline].value += dis[scaleType];
      stats[D1_StatHashes.Strength].value += str[scaleType];
    }

    switch (armor.bonusType) {
      case 'int':
        stats[D1_StatHashes.Intellect].value += int.bonus;
        break;
      case 'dis':
        stats[D1_StatHashes.Discipline].value += dis.bonus;
        break;
      case 'str':
        stats[D1_StatHashes.Strength].value += str.bonus;
        break;
    }
  }
}
...
export function getBestArmor(
  ...
  const statHashes = [
    { stats: [D1_StatHashes.Intellect, D1_StatHashes.Discipline], type: 'intdis' },
    { stats: [D1_StatHashes.Intellect, D1_StatHashes.Strength], type: 'intstr' },
    { stats: [D1_StatHashes.Discipline, D1_StatHashes.Strength], type: 'disstr' },
    { stats: [D1_StatHashes.Intellect], type: 'int' },
    { stats: [D1_StatHashes.Discipline], type: 'dis' },
    { stats: [D1_StatHashes.Strength], type: 'str' },
  ];
